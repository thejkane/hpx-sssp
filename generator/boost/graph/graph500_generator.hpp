...
namespace boost {
  ...
  template<typename vertices_size_type, typename edges_size_type>
  class graph500_iterator
    : public iterator_facade<graph500_iterator<vertices_size_type, edges_size_type>,
			     std::pair<vertices_size_type, vertices_size_type>,
			     std::input_iterator_tag,
			     const std::pair<vertices_size_type,
					     vertices_size_type>&>
  {
  public:
    ...
    graph500_iterator(vertices_size_type scale, edges_size_type start_edge, 
      ...
      : scale(scale), current(start_edge), packed_edges(new packed_edge[1 << 16]) //, flip(false)
    {
    ...
    void update_cache(bool run_from_increment) {
      /*if (flip) {
	current_edge = std::make_pair(current_edge.second, current_edge.first);
	flip = false;
	} else {*/
  ...
  private:
    ...
    //    bool flip; // generate undirected graph
